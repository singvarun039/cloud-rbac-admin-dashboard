    ...
    <Modal title="Edit user" isOpen={isOpen} onClose={onClose}>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <Label>Name</Label>
          <Input value={name} onChange={(e) => setName(e.target.value)} type="text" disabled={!canEdit || submitting || !user} className="h-10" />
        </div>

        <div className="space-y-2">
          <Label>Email</Label>
          <Input value={email} onChange={(e) => setEmail(e.target.value)} type="email" disabled={!canEdit || submitting || !user} className="h-10" />
        </div>

        <div className="space-y-2">
          <Label>Status</Label>
          <Select value={status} onValueChange={(value) => setStatus(value as UserStatus)} disabled={!canEdit || submitting || !user}>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button type="button" onClick={() => void onSubmit()} disabled={!canEdit || submitting || !user}>
