        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
          <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 lg:flex-1">
            <Select value={actionInput} onValueChange={setActionInput}>
              <SelectTrigger className="h-10 w-full" aria-label="Action"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="flex w-full flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-end lg:w-auto">
            ...
            <Button variant="outline" type="button" onClick={onResetFilters} disabled={loading} className="h-10 w-full sm:w-auto">Reset Filters</Button>
            <Separator orientation="vertical" className="hidden h-10 sm:block" />

            <Drawer open={advancedOpen} onOpenChange={setAdvancedOpen} direction="right">
              <DrawerTrigger asChild>
                <Button type="button" className="h-10 w-full sm:w-auto">Advanced Filters</Button>
              </DrawerTrigger>
              <DrawerContent className="inset-y-0 right-0 h-full w-3/4 border-l border-slate-200 sm:max-w-sm">
                <DrawerClose className="absolute right-2 top-2 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2" aria-label="Close">
                  <span className="text-lg leading-none">×</span>
                </DrawerClose>
                <div className="flex h-full flex-col px-6 pb-6 pt-4">
                  <DrawerHeader className="pr-10">
                    <DrawerTitle>Advanced Filters</DrawerTitle>
                    <DrawerDescription>Refine audit logs using additional fields.</DrawerDescription>
                  </DrawerHeader>
                  <div className="mt-4 flex-1 space-y-3 overflow-auto">
                    <Input value={actorUserIdInput} onChange={(e) => setActorUserIdInput(e.target.value)} placeholder="Actor user ID" aria-label="Actor user ID" type="text" className="h-10 w-full placeholder:text-slate-400" />
                    <Input value={entityTypeInput} onChange={(e) => setEntityTypeInput(e.target.value)} placeholder="Entity type" aria-label="Entity type" type="text" className="h-10 w-full placeholder:text-slate-400" />
                    <Input value={entityIdInput} onChange={(e) => setEntityIdInput(e.target.value)} placeholder="Entity ID" aria-label="Entity ID" type="text" className="h-10 w-full placeholder:text-slate-400" />
                  </div>
                  <DrawerFooter className="border-t border-slate-200 pt-4">
                    <Button variant="outline" type="button" onClick={onResetFilters} className="h-10">Reset</Button>
                    <Button type="button" onClick={() => { onApplyFilters(); setAdvancedOpen(false); }} className="h-10">Apply Filters</Button>
                  </DrawerFooter>
                </div>
              </DrawerContent>
            </Drawer>
