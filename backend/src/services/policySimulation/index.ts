...
export type PolicySimulationResult = {
  ...
  impacts: { losingAccess: SurfaceImpact[]; gainingAccess: SurfaceImpact[]; unchangedAccessible: SurfaceImpact[] };
  summary: string;
};
...
export async function simulateRolePolicyChange(input: {
  ...
  if (Array.isArray(input.permissionKeys) && input.permissionKeys.length > 0) {
    proposedPermissions = await prisma.permission.findMany({ where: { key: { in: input.permissionKeys } }, select: { id: true, key: true } });
    ...
    proposedPermissions = await prisma.permission.findMany({ where: { id: { in: input.permissionIds } }, select: { id: true, key: true } });
  ...
  const proposedPermissionKeys = proposedPermissions.map((p) => p.key).sort((a, b) => a.localeCompare(b));
  ...
  for (const surface of APP_ACCESS_SURFACES) {
    ...
    const shaped: SurfaceImpact = { kind: surface.kind, key: surface.key, label: surface.label, description: surface.description, requiredAnyOf: surface.requiredAnyOf };
  ...
  const summary = await generateSimulationSummary({ roleName: role.name, currentPermissionKeys, proposedPermissionKeys, removedPermissionKeys, addedPermissionKeys, losingAccess, gainingAccess });

  return { role: { id: role.id, name: role.name, description: role.description }, currentPermissionKeys, proposedPermissionKeys, addedPermissionKeys, removedPermissionKeys, impacts: { losingAccess, gainingAccess, unchangedAccessible }, summary };
}
