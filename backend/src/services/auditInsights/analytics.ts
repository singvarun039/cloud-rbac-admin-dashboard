...
type RecentFailureRow = {
  ...
  actorUserId: string | null; actorEmail: string | null; createdAt: string;
};
...
export async function getAuditInsightsAnalytics(windowDays: number) {
  ...
  const [trendRows, topActionRows, topActorRows, recentFailureRows, totalEvents] = await Promise.all([
  ...
  const totalFailures = recentFailureRows.length
    ? await prisma.auditLog.count({ where: { createdAt: { gte: start, lt: endExclusive }, action: { contains: "FAILURE" } } })
    : 0;

  const topActions = topActionRows.map((r) => ({ action: r.label ?? "unknown", count: Number(r.count) || 0 }));
  const topActors = topActorRows.map((r) => ({ actor: r.label ?? "unknown", count: Number(r.count) || 0 }));
  const peakDay = trend.length > 0 ? trend.reduce((best, day) => (day.count > best.count ? day : best), trend[0]) : null;
  const latestDay = trend.length > 0 ? trend[trend.length - 1] : null;
  const priorDays = trend.slice(0, -1).map((day) => day.count);
  const latestVsBaseline = latestDay ? { latestCount: latestDay.count, priorAverage: Number(average(priorDays).toFixed(2)) } : null;

  return { windowDays, trend, totalEvents, totalFailures, topActions, topActors, recentFailures: recentFailureRows, peakDay, latestDay, latestVsBaseline };
}
