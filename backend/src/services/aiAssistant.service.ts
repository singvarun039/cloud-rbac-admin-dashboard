...
function buildDeterministicAssistantReply(context: AssistantContext): string {
  ...
  const { latestDay, priorAverage, topRecentAction } = getAuditSignals(context.summary);
  const lines: string[] = [];

  if (lowerPrompt.includes('risk') || lowerPrompt.includes('anomal')) {
    lines.push('Observations:');

    if (latestDay && latestDay.count >= 4 && latestDay.count > Math.max(priorAverage * 2, 1)) {
      lines.push(
    ...
    if (!recentAudit.length) {
      lines.push(
        '- There is very little recent audit activity available to compare against a normal baseline.'
      );
    }
  ...
  if (
    ...
    lines.push('Recommendations:');
    lines.push(
      '- Review role design on the Roles page and confirm each role still maps cleanly to real job boundaries.'
    );
    lines.push(
  ...
  if (latestDay) {
    lines.push(
      `- The latest audit day is ${latestDay.date} with ${latestDay.count} recorded events.`
    );
  }

  if (topRecentAction) {
    lines.push(
      `- The most frequent recent action is ${topRecentAction.action} (${topRecentAction.count} in the latest activity list).`
    );
  }

  lines.push('Recommendations:');
  lines.push(
    '- Use the Roles page AI recommendations and policy simulation to review any broad or risky access patterns.'
  );
...
export async function generateAdminAssistantReply(context: AssistantContext): Promise<string> {
