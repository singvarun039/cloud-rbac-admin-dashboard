...
import { buildRoleRecommendationInstructions, buildDeterministicRoleRecommendations, isUsableRoleRecommendationAnswer, type RoleRecommendationContext } from "./deterministic";
...
export async function generateRoleRecommendations(input: { windowDays: number; includeAuditSignals: boolean }): Promise<RoleRecommendationsResult> {
  ...
  const rolesWithNoPermissions = roleSnapshots.filter((r) => r.permissions.length === 0).map((r) => r.name);
  const broadestRoles = [...roleSnapshots].sort((a, b) => b.permissions.length - a.permissions.length).slice(0, 5).map((r) => ({ role: r.name, permissionCount: r.permissions.length }));
  ...
  const ctx: RoleRecommendationContext = { windowDays: input.windowDays, roles: roleSnapshots, overlapPairs, rolesWithNoPermissions, broadestRoles, auditSignals, roleAuditVisible: input.includeAuditSignals };
  ...
  try {
    response = await fetch(`${env.OPENAI_API_BASE_URL}/responses`, {
      ...
      body: JSON.stringify({
        ...
        instructions: buildRoleRecommendationInstructions({ roleAuditVisible: input.includeAuditSignals }),
        input: [`Audit window for role behavior: last ${input.windowDays} days`, "", "Role recommendation context JSON:", JSON.stringify(ctx, null, 2)].join("\n"),
  ...
  const answer = isUsableRoleRecommendationAnswer(extracted) ? extracted : buildDeterministicRoleRecommendations(ctx);

  if (!answer) {
    throw new AppError(502, "AI_EMPTY_RESPONSE", "The AI provider returned an empty response.", summarizeOpenAiPayload(payload));
  }

  return { windowDays: input.windowDays, answer, analytics: { totalRoles: roleSnapshots.length, totalDistinctPermissions: distinctPermissions.size, rolesWithNoPermissions, broadestRoles, overlapPairs, auditSignals, roleAuditVisible: input.includeAuditSignals } };
}
