...
export type RoleOverlapPair = { roleA: string; roleB: string; sharedPermissions: string[]; overlapCount: number };
...
export function computeOverlapPairs(roles: RoleSnapshot[]): RoleOverlapPair[] {
  ...
  for (let i = 0; i < roles.length; i++) {
    for (let j = i + 1; j < roles.length; j++) {
      const left = roles[i]; const right = roles[j];
      ...
      pairs.push({ roleA: left.name, roleB: right.name, sharedPermissions, overlapCount: sharedPermissions.length });
    }
  }
...
export async function getRoleSnapshots() {
  const roles = await prisma.role.findMany({
    orderBy: { name: "asc" },
    select: { id: true, name: true, description: true, permissions: { select: { permission: { select: { key: true } } }, orderBy: { permission: { key: "asc" } } } },
  });
  return roles.map((r) => ({ id: r.id, name: r.name, description: r.description, permissions: r.permissions.map((p) => p.permission.key) }));
}
