...
export function buildDeterministicRoleRecommendations(context: RoleRecommendationContext): string {
  ...
  if (context.overlapPairs.length > 0) {
    ...
    summaryParts.push(
      `The highest overlap is between ${top.roleA} and ${top.roleB} with ${top.overlapCount} shared permission(s).`
    );
  }
  ...
  if (context.overlapPairs.length > 0) {
    const top = context.overlapPairs[0];
    recommendations.push(
      `Compare ${top.roleA} and ${top.roleB} to confirm whether both roles are still needed as separate access boundaries.`
    );
    risks.push(
      `High permission overlap between ${top.roleA} and ${top.roleB} may make the policy harder to maintain and review.`
    );
  }
  ...
  if (context.roleAuditVisible && context.auditSignals.length > 0) {
    recommendations.push(
      `Use the recent ${context.auditSignals[0].action} audit activity to sanity-check whether role changes are happening in a controlled way.`
    );
